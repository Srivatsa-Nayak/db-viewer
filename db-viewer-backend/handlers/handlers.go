...
func HandleGetTableData(c *gin.Context) {
	...
	// 1. Fetch Columns explicitly (So we have headers even if table is empty)
	var columns []string
	if database.CurrentDriver == "mysql" {
		...
		for rows.Next() {
			...
			var name string
			var rest interface{}
			rows.Scan(&cid, &name, &rest, &rest, &rest, &rest)
	...
	var tableRows []map[string]interface{} = []map[string]interface{}{} // Initialize as empty slice, NOT nil
	...
	// If empty, rows will be [] instead of null.
	c.JSON(http.StatusOK, gin.H{
		"columns": columns,
		"rows":    tableRows,
	})
