            ...
            {showClearConfirm && (
                <div className="fixed inset-0 bg-black/60 z-[100] flex items-center justify-center p-4 backdrop-blur-sm">
                    <div className="bg-white border border-zinc-200 rounded-lg p-6 max-w-md w-full shadow-2xl border-t-4 border-t-red-500">
                        <h3 className="text-lg font-bold text-zinc-900 flex items-center gap-2"><Trash2 size={20} className="text-red-500"/> Delete file?</h3>
                        <p className="text-zinc-500 text-sm mt-2 mb-4">
                            &quot;{activeWorkspace?.name}&quot; and its database will be permanently deleted,
                            along with every table and row in it. Any share link for this file stops working.
                            This cannot be undone.
                        </p>
                        <div className="flex gap-3 justify-end">
                            <button onClick={() => setShowClearConfirm(false)} className="px-4 py-2 text-zinc-500 hover:text-zinc-900">Cancel</button>
                            <button onClick={confirmClear} className="px-4 py-2 bg-red-600 hover:bg-red-700 text-white rounded">Delete file</button>
                        </div>
                    </div>
                </div>
            )}
